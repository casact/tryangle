...
def get_expected(estimator, X):
    ...
    return expected.to_frame().fillna(0).to_numpy().reshape(-1, 1)
...
def get_actual_expected(estimator, X):
    ...
    """
    # ! TODO: Rewrite a better method that avoids  fillna(0) as this could mask an error

    actual = X.latest_diagonal.to_frame().fillna(0).to_numpy()
    if X.triangle.shape[0] > 1:
        actual = actual[:, :-1].reshape(-1, 1)
    else:
        actual = actual[:-1]
